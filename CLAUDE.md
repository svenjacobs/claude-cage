...
Finally, when the keys are mounted and a `user.signingkey` is configured, the entrypoint runs `gpg-unlock` once **at startup** (before `exec claude`) to prime the passphrase cache, so Claude's non-interactive commits can sign without a per-commit prompt. This is non-fatal: skipping or cancelling the prompt still launches Claude.

### `bin/gpg-unlock`

A helper on the image `PATH`. Because Claude runs `git` non-interactively and cannot type the key passphrase, the key is unlocked once per session — automatically at startup by the entrypoint, or manually by running `gpg-unlock` in an interactive shell. It sets `GPG_TTY`, then makes one throwaway signature (with `user.signingkey`) to trigger the `pinentry-tty` prompt and populate the shared `gpg-agent` cache; Claude's later commits reuse the cached passphrase (held for the long TTL above). It is inert if no signing key is present or there is no terminal.
